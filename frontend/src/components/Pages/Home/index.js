...
export default function Home({ }) {
	...
	const [countLaunch, setCountLaunch] = useState()

	async function getLaunch() {
		const response = await api.get('launchs')
		let num = 0;
		for (let index = 0; index < response.data.length; index++) {
			num = parseFloat(response.data[index].launch + num);
		}
		setLaunch(num)
	}
			...
			<div
				...
				<div style={{
					...
					<div style={{
						...
						<CardExpenditure >
							<div>
								<strong style={{ color: 'white', fontSize: '2em' }}>{launch.toLocaleString('pt-br', { style: 'currency', currency: 'BRL' })}</strong>
